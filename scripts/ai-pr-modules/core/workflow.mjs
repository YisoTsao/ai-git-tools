...
import { LabelAnalyzer } from '../ai/label-analyzer.mjs';
import { ReviewerSelector } from '../reviewers/reviewer-selector.mjs';
import { Logger } from '../ui/logger.mjs';
import { PRError, log } from '../utils/helpers.mjs';
    ...
    if (!baseBranch) {
      ...
      if (this.config.github?.defaultBase) {
        ...
        if (defaultBase === 'release' || defaultBase.match(/^release$/i)) {
          ...
          if (latestRelease) {
            ...
            throw new PRError('未偵測到任何 release 分支', 'NO_RELEASE_BRANCH', [
              '使用 --base 參數指定目標分支',
              '或在 .ai-pr-config.mjs 中設置具體的分支名（如 release-2025-m12.1）',
        ...
        if (latestRelease) {
          ...
          throw new PRError('未偵測到任何 release 分支', 'NO_RELEASE_BRANCH', [
            '使用 --base 參數指定目標分支',
            '或在 .ai-pr-config.mjs 中設置 github.defaultBase',
            '確認遠端分支存在: git branch -r | grep release',
          ]);
        }
      }
